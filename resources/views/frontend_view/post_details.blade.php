...
@section('meta')
  ...
  <meta property="og:title" content="{{$show_post->title_bn}}" />
  <meta property="og:description" content="{{$show_post->details_bn}}" />
  <meta property="og:image" content="{{asset('public/uploads/post')}}/{{$show_post->image}}" />
@endsection
@section('front_content')
  <!-- Start News Details Area -->
  <div class="container pl-5 pr-5">
  @php
     $advertisements= DB::table('advertisements')->where('ads_type',3)->orderBy('id','ASC')->skip(2)->take(1)->get(); 
  @endphp
        ...
        <div class="row">
            <div class="col-lg-8 col-md-12">
                <div class="blog-details-desc">
                    ...
                    <div class="article-image">
                        <img src="{{asset('public/uploads/post')}}/{{$show_post->image}}" alt="image" style="width: 800px; height: 550px;">
                    </div>

                    <div class="article-content">
                    <span><a href="#">{{$writer->bangla_name}}</a> / {{ \Carbon\Carbon::parse($show_post->created_at)->format('j F, Y')}} / <a href="#">{{$show_post->view}} View</a></span>
                        <h3 style="font-size: 30px; font-family:SolaimanLipi; line-height: 38px; font-weight: 500;">{{$show_post->title_bn}}</h3>
                        <p style="font-size: 16px; font-family: SolaimanLipi; font-weight: 400; line-height: 38px; color: rgb(8, 8, 8);" >{{$show_post->details_bn}}</p>
                    </div>
                        ...
                        <div class="health-news-slides owl-carousel owl-theme">
                            ...
                            <div class="single-health-news">
                                ...
                                <div class="health-news-image">
                                    <a href="#">
                                        <img src="{{asset('public/uploads/post')}}/{{$related_post->image}}" alt="image" style="width: 400px; height: 230px;">
                                    </a>
                                </div>
                                ...
                                <div class="health-news-content">
                                    <span></span>
                                    <h3>
                                        <a href="{{route('postshow.show',$related_post->slug_bn)}}">{{$related_post->title_bn}}</a>
                                    </h3>
                                </div>
                            </div>
                                @endforeach
            ...
            <div class="col-lg-4">
                <aside class="widget-area">
                    ...
                    @foreach ($popular_newses as $popular_news)
                        <article class="item" style="margin-top: -20px">
                            ...
                            <div class="info">
                                ...
                                <h3 style="font-size: 11px; font-name: SolaimanLipi; font-weigth: 400; line-height: 18px; margin-top: -7px;">
                                    <a href="#" style="color:rgb(10, 10, 10)">{!!str_limit($popular_news->details_bn,'90','...')!!}</a>
                                </h3>
                            </div> 
                    ...
                    @php
                    $advertisements= DB::table('advertisements')->orderBy('id','ASC')->where('ads_type',2)->skip(6)->take(1)->get(); 
                    @endphp
                    @foreach ($advertisements as $advertisement)
                    <div class="row">
                        <div class="container">
                    ...
                    @php
                    $advertisements= DB::table('advertisements')->select('advertisements.*')->where('ads_type',2)->orderBy('id','ASC')->skip(7)->take(1)->get(); 
                    @endphp
                    @foreach ($advertisements as $advertisement)
                    <div class="row">
                        <div class="container">
