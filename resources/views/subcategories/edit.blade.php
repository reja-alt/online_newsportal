                                ...
                                <label class="control-label">category Bangla Name</label>
                                    <select name="cat_name" class="form-control">
                                        @foreach($categories as $category)
                                          <option value="{{$category->id}}" @if($category->id == $subcategory->category_id) selected="" @endif >{{$category->name_bn}} | {{$category->name_en}} </option>     
                                        @endforeach
                                    </select>
                            </div>
                            <div class="form-group">
                                <label class="control-label" for="name"> Subcategory Bangla Name <span class="m-l-5 text-danger"> *</span></label>
                                <input class="form-control" type="text" name="name_bn" id="name_bn" value="{{$subcategory->name_bn }}"/>
                            </div>
                            <div class="form-group">
                                <label class="control-label" for="name">Subcategory English Name <span class="m-l-5 text-danger"> *</span></label>
                                <input class="form-control" type="text" name="name_en" id="name_en" value="{{ $subcategory->name_en }}"/>
                            </div>
                            <div class="form-group label-floating">
                                <label class="control-label">Status</label>
                                    <select id="show_on_header" name="show_on_header" required class="form-control">
                                        <option>-----Select-----</option> 
                                        <option value="1" @if($subcategory->show_on_header ==1) selected="" @endif>Active </option>
                                        <option value="0" @if($subcategory->show_on_header ==0) selected="" @endif>Inactive </option>
                                    </select>
                            </div>
