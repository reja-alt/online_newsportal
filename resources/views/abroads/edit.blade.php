...
<div class="container p-10"> 
    <div class="row">
        <div class="col-md-10  col-sm-10 mx-auto">
            <div class="tile">
                <form action="{{ route('abroad.update',$abroad->id) }}" method="POST" role="form" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    @if (count($errors) > 0)
                    ...
                    <div class="card">
                        <div class="card-body">  
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="title_bn" id="title_bn" value="{{ $abroad->title_bn }}"/>
                            </div>
                            <div class="form-group">
                                <label class="control-label" for="title_en">Title English Name <span class="m-l-5 text-danger"> *</span></label>
                                <input class="form-control" type="text" name="title_en" id="title_en" value="{{ $abroad->title_en }}"/>
                            </div>
                            <div class="form-group">
                                <label class="control-label" for="details_bn">Details Bangla<span class="m-l-5 text-danger"> *</span></label>
                                <textarea class="form-control" id="summernote" name="details_bn">{{$abroad->details_bn}}</textarea>
                            </div>                  
                            <div class="form-group">
                                <label class="control-label" for="details_en">Details English<span class="m-l-5 text-danger"> *</span></label>
                                <textarea class="form-control" id="summernote2" name="details_en">{{$abroad->details_en}}</textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control " type="date" name="published_date" id="published_date" value="{{$abroad->published_date}}"/>
                            </div> 
                            ...
                            <div class="form-group label-floating">
                                <label class="control-label">Status</label>
                                    <select id="status" name="status"   class="form-control" >
                                        <option value="1" @if($abroad->status==1) selected="" @endif >Active </option>
                                        <option value="0"  @if($abroad->status==0) selected="" @endif>Inactive </option>
                                    </select>
                            </div>
                            <div class="tile-footer">
                                ...
                                &nbsp;&nbsp;&nbsp;
                                    <a class="btn btn-sm btn-danger btn-square btn-uppercase" href="{{ route('abroad.index') }}"><i class="fa fa-fw fa-sm fa-times-circle"></i>Cancel</a>
                            </div>
                        </div>
                    </div>
                </form>
